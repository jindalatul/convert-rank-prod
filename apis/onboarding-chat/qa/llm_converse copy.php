...
function save_project_simple($user_id, $project_name, $persona_json, $seed_keywords_json) {
  global $mysqli;
  $user_id = (int)$user_id;
...
function next_key($order, $full_answers) {
  foreach ($order as $k) if (!array_key_exists($k, $full_answers)) return $k;
  return null;
}
function step_index($order, $full_answers) {
  $n=0; foreach ($order as $k){ if(array_key_exists($k,$full_answers)) $n++; else break; } return $n;
}
function merge_answer($full_answers, $current, $isMulti) {
  $key = $current['key'] ?? null; if(!$key) return $full_answers;
  ...
  $full_answers[$key]=['key'=>$key,'display_value'=>$display,'status'=>'answered','selected_options'=>$selected,'custom_options'=>$custom];
  return $full_answers;
}
function compact_answers($order, $templates, $full_answers) {
  $out=[]; foreach($order as $k){ if(!isset($full_answers[$k])) continue;
    $rec=$full_answers[$k]; $isMulti=!empty($templates[$k]['multi']);
...
function build_prompt($MASTER_PROMPT,$templatesArr,$answersArr,$stateArr){
  ...
  return str_replace(['{$templatesJson}','{$answersJson}','{$stateJson}'],[$templatesJson,$answersJson,$stateJson],$MASTER_PROMPT);
}
// strip code fences and pick innermost plausible JSON
...
function call_gemini($api_key,$model,$prompt_text,$timeout=20){
  ...
  // FIXED: correct CURLOPT_POSTFIELDS usage (no stray quote)
  curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($payload));
  curl_setopt($ch, CURLOPT_TIMEOUT, $timeout);
  $resp=curl_exec($ch);
  if ($resp === false) return ['text'=>'','raw'=>null];
  curl_close($ch);
  $data=json_decode($resp,true);
  $text=$data['candidates'][0]['content']['parts'][0]['text']??'';
  return ['text'=>$text,'raw'=>$data];
}
function normalize_next($obj,$templateForTarget){
  $mode=in_array(($obj['mode']??''),['text','chips+input'],true)?$obj['mode']:$templateForTarget['mode'];
  $multi=(bool)($obj['multi']??$templateForTarget['multi']);
  $options=is_array($obj['options']??null)?array_values(array_filter($obj['options'],fn($x)=>trim((string)$x)!='')):[];
  $out=['key'=>(string)($obj['key']??$templateForTarget['key']),'q'=>(string)($obj['q']??$templateForTarget['q']),
        'hint'=>(string)($obj['hint']??$templateForTarget['hint']),'mode'=>$mode,'multi'=>$multi,'allowCustom'=>true,'options'=>$options];
  if($out['key']==='seed_keywords') $out['maxChoices']=4;
  return $out;
}

// ===== One-time REPEAT memory (file-based; per IP+UA, 10 min) =====
function fp_path(){
  $dir=sys_get_temp_dir().'/onboard_fp';
  if(!is_dir($dir)) @mkdir($dir,0777,true);
  $ip=$_SERVER['REMOTE_ADDR']??'0.0.0.0';
  $ua=$_SERVER['HTTP_USER_AGENT']??'';
  $fp=hash('sha1', $ip.'|'.$ua);
  return [$dir, $dir.'/'.$fp.'.json'];
}
function get_repeat_state(){ [, $path]=fp_path(); if(!file_exists($path)) return ['ts'=>time(),'clarified'=>[]];
  $raw=@file_get_contents($path); $j=json_decode($raw,true); if(!is_array($j)) return ['ts'=>time(),'clarified'=>[]];
  if(($j['ts']??0) < time()-600) return ['ts'=>time(),'clarified'=>[]]; // 10 min expiry
  return $j;
}
function set_repeat_state($state){ [, $path]=fp_path(); $state['ts']=time(); @file_put_contents($path,json_encode($state)); }
...
// INIT — force first key, rewrite with Gemini or fallback
if ($action === 'INIT') {
  $firstKey = $QA_ORDER[0];
  $prompt = build_prompt($MASTER_PROMPT, $QA_TEMPLATES, new stdClass(), ['asked_keys'=>[], 'max_questions'=>6, 'total_templates'=>count($QA_TEMPLATES)]);
  $gem = call_gemini($GEMINI_API_KEY, $GEMINI_MODEL, $prompt, $G_TIMEOUT_SEC);

  $next = $QA_TEMPLATES[$firstKey];
  if (!empty($gem['text'])) {
    $clean = extract_json($gem['text']);
    $obj = json_decode($clean, true);
    if (is_array($obj) && !empty($obj['next'])) {
      $cand = normalize_next($obj['next'], $next);
      if (($cand['key'] ?? '') === $firstKey) $next = $cand;
    }
  }
  send(['ok'=>true,'status'=>'NEXT','step'=>0,'next'=>$next]);
}

// ANSWER — one-time clarify; otherwise advance; save at end
if ($action === 'ANSWER') {
  $current = $body['current'] ?? [];            // { key, text, selected[], custom[] }
  $context = $body['context'] ?? [];            // { full_answers, skip? }  (skip ignored here)
  ...
  // one-time REPEAT
  $state = get_repeat_state();
  $alreadyClarified = in_array($ckey, $state['clarified'], true);
  $invalid = is_invalid_answer($ckey, $current, $isMulti);

  if ($invalid && !$alreadyClarified) {
    $state['clarified'][] = $ckey; set_repeat_state($state);

    // ask concise clarification for SAME key (Gemini rewrite or fallback)
    $answersCompact = compact_answers($QA_ORDER, $QA_TEMPLATES, $full);
    $prompt = build_prompt($MASTER_PROMPT, $QA_TEMPLATES, $answersCompact,
      ['asked_keys'=>array_keys($answersCompact), 'max_questions'=>6, 'total_templates'=>count($QA_TEMPLATES)]
    );
    $gem = call_gemini($GEMINI_API_KEY, $GEMINI_MODEL, $prompt, $G_TIMEOUT_SEC);

    $next = $QA_TEMPLATES[$ckey];
    if (!empty($gem['text'])) {
      $clean = extract_json($gem['text']);
      $obj = json_decode($clean, true);
      if (is_array($obj) && !empty($obj['next'])) {
        $cand = normalize_next($obj['next'], $next);
        if (($cand['key'] ?? '') === $ckey) $next = $cand;
      }
    }
    send(['ok'=>true,'status'=>'REPEAT','step'=>step_index($QA_ORDER, $full),'next'=>$next]);
  }

  // accept (even if still weak) and advance
  ...
  if ($targetKey === null) {
    // save and DONE
    $answersCompact = compact_answers($QA_ORDER, $QA_TEMPLATES, $full);
    $seedArray = $answersCompact['seed_keywords'] ?? [];
    if (!is_array($seedArray)) $seedArray = array_map('trim', explode(',', (string)$seedArray));
    $seedArray = array_values(array_filter($seedArray, fn($x)=>$x!==''));

    $persona = ['answers'=>$answersCompact, 'raw'=>$full, 'created_at'=>gmdate('c')];
    $persona_json = json_encode($persona, JSON_UNESCAPED_UNICODE);
    $seed_json    = json_encode($seedArray, JSON_UNESCAPED_UNICODE);

    $user_id = 1;
    $project_name = 'Content Strategy Project ' . gmdate('Y-m-d');

    $pid = save_project_simple($user_id, $project_name, $persona_json, $seed_json);
    send(['ok'=>true,'status'=>'DONE','project_id'=>$pid,'message'=>'Saved']);
  }

  // ask next target key (Gemini rewrite or fallback)
  $answersCompact = compact_answers($QA_ORDER, $QA_TEMPLATES, $full);
  $prompt = build_prompt($MASTER_PROMPT, $QA_TEMPLATES, $answersCompact,
    ['asked_keys'=>array_keys($answersCompact), 'max_questions'=>6, 'total_templates'=>count($QA_TEMPLATES)]
  );
  ...
  $next = $QA_TEMPLATES[$targetKey];
  if (!empty($gem['text'])) {
    $clean = extract_json($gem['text']);
    $obj = json_decode($clean, true);
    if (is_array($obj) && !empty($obj['next'])) {
      $cand = normalize_next($obj['next'], $next);
      if (($cand['key'] ?? '') === $targetKey) $next = $cand;
    }
  }

  $step = step_index($QA_ORDER, $full);
  send(['ok'=>true,'status'=>'NEXT','step'=>$step,'next'=>$next]);
}
